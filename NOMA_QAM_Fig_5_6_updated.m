%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This MATLAB script is used to generate Figure 5 and 6 in the paper:
%
...
%% SER of the first user with a1=0.85, M1=4, and the second user with M2=4,16, and 64.
...
color1 = [dblue; dred; lgreen];
color2 = [lblue; lred; green]; 
color3 = [blue; red; dgreen];
...
for i = 1:length(M2)
[ser1t, ser2t] = Theoretical_SER(M1,M2(i),a1,SNR_dB);
% [ser1a, ser2a] = Approximated_SER(M1,M2(i),a1,SNR_dB);
[ser1s, ser2s] = Simulated_SER(M1,M2(i),a1,SNR_dB(1:2:end));

figure(1)
semilogy(SNR_dB, ser1t, '-', 'color', color1(i,:), 'linewidth', 1); 
hold on;
% semilogy(SNR_dB, ser1a, '--', 'color', color2(i,:), 'linewidth', 1);
semilogy(SNR_dB(1:2:end), ser1s, style(i), 'color', color3(i,:), ...
    'linewidth', 1); 

figure(2)
semilogy(SNR_dB,ser2t,'-', 'color', color1(i,:), 'linewidth', 1); 
hold on;
% semilogy(SNR_dB, ser2a, '--', 'color', color2(i,:), 'linewidth', 1);
semilogy(SNR_dB(1:2:end), ser2s, style(i), 'color', color3(i,:), ...
    'linewidth', 1);
end

figure(1)
grid on; xlabel('SNR (dB)', 'Interpreter', 'latex');
ylabel('$SER_1$', 'Interpreter', 'latex');
axis([0 22 1e-5 1]);
% legend('$M_1=4$, $M_2=4$ (Theory)', '$M_1=4$, $M_2=4$ (Approx.)',...
%     '$M_1=4$, $M_2=4$ (Simu.)', '$M_1=4$, $M_2=16$ (Theory)', ...
%     '$M_1=4$, $M_2=16$ (Approx.)', '$M_1=4$, $M_2=16$ (Simu.)',...
%     '$M_1=4$, $M_2=64$ (Theory)', '$M_1=4$, $M_2=64$ (Approx.)',...
%     '$M_1=4$, $M_2=64$ (Simu.)','location','sw')
legend('$M_1=4$, $M_2=4$ (Theory)', '$M_1=4$, $M_2=4$ (Simulation)', ...
    '$M_1=4$, $M_2=16$ (Theory)', '$M_1=4$, $M_2=16$ (Simulation)', ...
    '$M_1=4$, $M_2=64$ (Theory)', '$M_1=4$, $M_2=64$ (Simulation)', ...
    'location','sw')
...
% To reduce simulation time, set maxBits = 1e5.
...
for n = 1:length(SNR_dB)
    ...
    while errorStats1(2) < maxErrs && errorStats1(3) < maxBits
        ...
        dataMod_1 = qammod(dataIn_1,M1,UnitAveragePower=true);
        dataMod_2 = qammod(dataIn_2,M2,UnitAveragePower=true);
        
        % Total electrical signal
        x = sqrt(a1)*dataMod_1 + sqrt(a2)*dataMod_2;
        ...
        dataOut1 = qamdemod(out,M1,UnitAveragePower=true);
        errorStats1 = errorRate1(dataIn_1,dataOut1);
        
        y2 = out - sqrt(a1)*qammod(dataOut1,M1,UnitAveragePower=true'); % Interference cancelation
        dataOut2 = qamdemod(y2/sqrt(a2),M2,UnitAveragePower=true); % Note the power of y2 should be normalized to 1 (by dividing by sqrt(a2)) to use qamdemod function with UnitAveragePower.
